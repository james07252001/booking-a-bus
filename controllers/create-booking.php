...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && count($_POST) > 0) {
    if ($_POST['type'] == 1) {
        ...
        if (isset($_FILES['upload_id']) && $_FILES['upload_id']['error'] === UPLOAD_ERR_OK) {
            $uploadDir = 'uploads/';
            $fileName = uniqid() . "_" . basename($_FILES['upload_id']['name']); // Unique filename
            ...
            if (move_uploaded_file($_FILES['upload_id']['tmp_name'], $uploadFile)) {
                $upload_id = $uploadFile; // Save the file path
            } else {
