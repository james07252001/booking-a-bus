...
if (isset($_POST["sign-up-submit"])) {
    ...
    $terms_agreed = isset($_POST["terms"]) ? true : false;
     ...
     if (!preg_match($name_pattern, $first_name)) {
         ...
         if (strtolower($first_name) === 'admin' || strtolower($last_name) === 'admin') {
             // Block registration if username is "admin"
             $ip_address = $_SERVER['REMOTE_ADDR'];
             $blocked_ips = file_get_contents('blocked_ips.txt'); // A simple file to store blocked IPs
             $blocked_ips = explode("\n", $blocked_ips);
             
             // If the current IP is not already blocked, block it
             if (!in_array($ip_address, $blocked_ips)) {
                 file_put_contents('blocked_ips.txt', $ip_address . "\n", FILE_APPEND);
             }
             
             $error = "You are not allowed to register with the name 'admin'. Your IP has been blocked.";
         } else {
